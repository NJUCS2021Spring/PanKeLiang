...
Group* WordBook::FileRead(Group* head) {
	FILE* pfile = fopen("D:\\MyCode\\Word.docx", "r");
	if (pfile == NULL)
		cout << "Error!";
	else {
		Group* p = new Group;
		while (fscanf(pfile, "%d %s", &p->id, p->reason) != EOF) {
			Word* t = new Word;
			while (fscanf(pfile, "%s %s %s", t->word_name, t->meaning,
